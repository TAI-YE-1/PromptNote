...
本文件定义 PromptNote 当前用户体验主链和交互边界。
...
PromptNote 不向第三方网页输入框自动写入或自动发送内容。
...
- 内容范围；
- 测试连接；
- 补全模型、上下文长度、触发延迟等高级设置。

Provider、Model、credential 和补全设置属于 Extension Preferences，不属于 PromptDocument。
...
核心交互必须在浏览器实际允许的窄 Side Panel 下保持可用，不产生必须横向滚动才能完成的主操作。
