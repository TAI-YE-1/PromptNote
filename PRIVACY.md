...
PromptNote / 提词笺是一款本地优先的 Chrome / Edge Side Panel Prompt 编辑器。本说明解释 PromptNote 1.0.0 会保存什么数据、什么时候访问网络，以及如何删除数据。

## 1. 本地保存的数据

PromptNote 使用 `chrome.storage.local` 保存：
...
PromptNote 没有自有账号系统、云数据库或后端，因此不会把这些数据同步到 PromptNote 自有服务器。

`chrome.storage.local` 是浏览器扩展本地存储，不是专用加密保险库。
...
请求直接发送到用户配置的 AI Provider / API Base URL，PromptNote 自身不代理这些请求。
...
如需删除全部本地数据，可以移除扩展并清除该扩展的本地数据。不同浏览器版本的入口名称可能略有差异。

## 6. 隐私说明变更

如果未来版本新增账号、云同步、遥测、PromptNote 自有后端或新的数据用途，本说明必须在对应版本发布前更新。
