...
describe('section insertion transactions', () => {
  it('changes an existing semantic section in place at block start', () => {
    const section = schema.nodes.promptSection.create({ kind: 'context' }, schema.text('背景正文'))
    ...
    expect(next.doc.firstChild?.textContent).toBe('背景正文')
  })

  it('converts a top-level paragraph in place at block start', () => {
