...
describe('Browser / Desktop backup compatibility', () => {
  ...
  it('keeps Browser and Desktop persistence local and separate', () => {
    ...
    expect(desktopStorage).toContain('CREATE TABLE prompt_documents')
    expect(desktopStorage).toContain('PRAGMA user_version = 1')
    expect(desktopHost).toContain('data_dir.join("promptnote.sqlite3")')
